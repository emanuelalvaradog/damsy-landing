  ...
  function showCreatedHistory() {
    if (createdHistory.length === 0) {
      ...
      createdHistory.forEach((element) => {
        objsToRender.push(
          <div className={styles.historyItem} key={Math.random()}>
            <h1>{element.query}</h1>
      ...
      return <div className={styles.historyList}>{objsToRender}</div>;
    }
  }
        ...
        <div
          ...
          className={
            currentScreen === Screens.CREATE_FORMULA
              ? styles.historyButton + " " + styles.historyButtonSelected
